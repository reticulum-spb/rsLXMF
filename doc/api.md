...
Это исторический аудит публичной поверхности, выполненный перед миграцией.
Он сохраняется как обоснование решений по совместимости; формулировки ниже
«необходимо», «план» и «при SQLite» описывают исходные требования, а не
незавершённую реализацию. Актуальное устройство storage и эксплуатационные
параметры описаны в `README_SQLITE.md` и основном `README.md`.

Миграция завершена: production daemon хранит долговременное runtime-состояние
в SQLite, файловая совместимость старого storage не поддерживается, активные
link/session objects остаются в памяти. Сохраняемые внешние интерфейсы
`rsNomadNet` не потребовали изменения.
